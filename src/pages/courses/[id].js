...
//public images
import Student from 'public/images/icon-student.svg'
...
export default function DetailCourse({data}) {
    ...
    useEffect(() => {
        const stickyOffestTop = footer.current.getBoundingClientRect().top
        const stickyMetaToogler = () => {
            setIsSticky(stickyOffestTop >= window.pageYOffset + window.innerHeight)
        }

        window.addEventListener("scroll", stickyMetaToogler)
        return () => {
            window.removeEventListener("scroll", stickyMetaToogler)
        }
    }, [])
    return (
        <>
            ...
            <section className="pt-10 relative overflow-hidden" style={{height:660}}>
                {
                    ...
                    <div className="video-wrapper">
                        <Youtube videoId={data?.chapters?.[0]?.lessons?.[0]?.video}
                        ...
                        <div className="meta-price w-full bg-white z-50 left-0 py-3">
                            <div className="w-3/4 mx-auto">
                                <div className="flex items-center">
                                    ...
                                    <a href={`${process.env.NEXT_PUBLIC_MEMBERPAGE_URL}/joined/${data.id}`} target="_blank" rel="noopener noreferrer" 
                                    className="bg-orange-500 hover:bg-orange-400 transition-all duration-200 focus:outline-none shadow-inner text-white
                    ...
                    <section className="mt-10 w-2/3">
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                ...
                                <h3 className="text-sm text-gray-600">{data?.mentor?.profession ?? "Mentor's Profession"} </h3>
                            </div>
                        </div>
                    </section>
