...
export default function Item({name, id, child, active, toggle, children}) {
    ...
    return (
        <div className="bg-white border border-gray-300 overflow-hidden">
            <div className="meta relative flex pl-4 py-2 justify-between items-center">
                ...
                <button className="link-wrapped pr-5 focus:outline-none" onClick={() => toggle(id)}>
                    {
                        ...
                        <ArrowDown className={['transition-all duration-300 transform', 
                        active === id ? "rotate-180" : "rotate-0"].join(" ")}>
                        </ArrowDown>
                    }
                </button>
            </div>
